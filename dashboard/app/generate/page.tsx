...
export default function GenerateDocumentsPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const traces = tracesContent.split('\n').filter(line => line.trim()).map(line => JSON.parse(line));
        ...
        {!downloadUrl ? (
          <>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <h3 className="font-semibold text-zinc-900 dark:text-white">{doc.name}</h3>
                          {doc.required && (
                        ...
                        <p className="text-xs text-zinc-500 dark:text-zinc-500 mt-1">{doc.pages}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <Button
              ...
              disabled={!tracesFile || !systemName || !providerName || isGenerating}
